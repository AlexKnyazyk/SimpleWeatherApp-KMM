...
interface IGetWeatherUseCase {
    suspend operator fun invoke(request: WeatherRequest): CResult<WeatherModel>
}
...
internal class GetWeatherUseCase(
    ...
    override suspend operator fun invoke(request: WeatherRequest): CResult<WeatherModel> {
        return when (request) {
            ...
            is WeatherRequest.FavoriteId -> {
                favoriteLocationsRepository.getById(request.id)?.let { favoriteModel ->
                    weatherRepository.getCurrentWeather(favoriteModel.name)
                        .onSuccess { weatherModel ->
                            updateFavoriteLocationWeatherUseCase(favoriteModel, weatherModel)
                        }
                } ?: CResult.failure(FavoriteLocationNotFoundError())
            }
